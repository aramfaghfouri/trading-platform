#!/usr/bin/env python3
"""
Clean professional TradingView-style chart with proper candlestick display.
Fixes the messy green bars issue with proper data formatting.
"""
import asyncio
import pandas as pd
import sys
...
class CleanProfessionalChart:
    """Clean professional chart with proper candlestick display."""
    ...
    def _create_clean_chart(self):
        """Create a clean professional candlestick chart."""
        logger.info("📊 Creating clean professional chart...")
        
        # Create clean sample data with proper OHLC format
        ...
        # Create chart with proper candlestick data
        ...
        logger.info(f"📈 Clean professional chart created with {len(chart_df)} candlesticks")
    ...
    async def start(self):
        """Start the clean professional chart."""
        logger.info(f"🚀 Starting clean professional chart for {self.symbol}")
        
        # Create data panel
        self._create_data_panel()
        
        # Create clean chart
        self._create_clean_chart()
        
        # Start real-time streaming
        self.running = True
        self._start_ibkr_streaming()
        
        logger.info(f"📊 Clean professional chart started!")
    ...
    def stop(self):
        ...
        if self.root:
            self.root.quit()
        logger.info("🔌 Clean professional chart stopped")

async def main():
    """Main function."""
    import argparse
    
    parser = argparse.ArgumentParser(description="Clean Professional Trading Chart")
    ...
    chart = CleanProfessionalChart(args.symbol)
