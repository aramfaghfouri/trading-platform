...
CREATE INDEX IF NOT EXISTS idx_strategy_state_running 
    ON strategy_state (is_running, last_heartbeat);

COMMENT ON TABLE strategy_state IS 'Tracks the current state of running strategies';
COMMENT ON COLUMN strategy_state.strategy_name IS 'Unique name of the strategy instance';
COMMENT ON COLUMN strategy_state.last_processed_timestamp IS 'Last bar timestamp processed by this strategy';
COMMENT ON COLUMN strategy_state.is_running IS 'Whether the strategy is currently running';
COMMENT ON COLUMN strategy_state.last_heartbeat IS 'Last heartbeat timestamp from the strategy';
COMMENT ON COLUMN strategy_state.error_message IS 'Most recent error message, if any';
...
CREATE TABLE IF NOT EXISTS strategy_signals (
    ...
    signal_type VARCHAR(20) NOT NULL,  -- 'BUY', 'SELL', 'HOLD', etc.
    confidence DECIMAL(5,4) DEFAULT 1.0,
    metadata JSONB,
    order_id VARCHAR(50),  -- Link to orders table if executed
    created_at TIMESTAMPTZ DEFAULT NOW(),
    
    CONSTRAINT fk_strategy_state 
        FOREIGN KEY (strategy_name) 
        REFERENCES strategy_state(strategy_name) 
        ON DELETE CASCADE
);

CREATE INDEX IF NOT EXISTS idx_signals_strategy_time 
    ON strategy_signals(strategy_name, signal_time DESC);
...
COMMENT ON TABLE strategy_signals IS 'Logs all trading signals generated by strategies';
COMMENT ON COLUMN strategy_signals.signal_type IS 'Type of signal: BUY, SELL, HOLD, CLOSE_LONG, CLOSE_SHORT';
COMMENT ON COLUMN strategy_signals.confidence IS 'Signal confidence level (0.0 to 1.0)';
COMMENT ON COLUMN strategy_signals.metadata IS 'Additional signal metadata (indicators, prices, etc.)';
COMMENT ON COLUMN strategy_signals.order_id IS 'Link to executed order if the signal resulted in an order';
